...
createBucketFolder(bucketAndFolder) {
...
initialize_from_S3() {
  ...
  if [ ! -f ${MOUNTS_HOME}/license/jasperserver.license -a aws s3 ls s3://${S3_LICENSE}/jasperserver.license ] ; then
...
save_jrsks_to_S3() {
  ...
  if [ "$S3_JRSKS_REQUIRED" = "true" -a ("$JRSKS_ORIG_DATE" -ne "$JRSKS_NEW_DATE") ] ; then
