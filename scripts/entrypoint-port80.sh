...
config_ssl() {
  ...
  if "$JRS_HTTPS_ONLY" = "true" ; then
    ...
    sed -i "s/80/443/g" js.quartz.properties
  else
	  ...
	  if [[ $KEYSTORE_PATH_FILES -ne 0 ]]; then
		  ...
		  for keystore in $KEYSTORE_PATH_FILES; do
			if [[ -f "$keystore" ]]; then
			  ...
			  cp "${keystore}" /root
			  xmlstarlet ed --inplace --subnode "/Server/Service/Connector[@port='443']" --type elem \ 
					--var connector-ssl '$prev' \
				--update '$connector-ssl' --type attr -n port -v "443" \
