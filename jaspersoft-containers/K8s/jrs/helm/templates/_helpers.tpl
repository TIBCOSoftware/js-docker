...
{{- if and .Values.scalableQueryEngine.enabled (not .Values.scalableQueryEngineUrl ) }}
 - name: "SCALABLE_QUERY_ENGINE_ENABLED"
   value: "true"
 - name: SCALABLE_QUERY_ENGINE_URL
   value: "http://{{ .Release.Name }}-query-engine-ingress.{{ .Release.Namespace }}.svc.cluster.local/query-engine"
 {{- else if .Values.scalableQueryEngineUrl }}
 - name: "SCALABLE_QUERY_ENGINE_ENABLED"
   value: "true"
 - name: SCALABLE_QUERY_ENGINE_URL
   value: {{ .Values.scalableQueryEngineUrl }}
{{- end }}
{{- if .Values.timeZone }}
 - name: TZ
...
volumeMounts:
  ...
  - mountPath: /usr/local/share/jasperserver-pro/keystore
    ...
    readOnly: true
  {{- if .Values.db.env }}
  - mountPath: /usr/local/tomcat/webapps/jasperserver-pro/META-INF
    name: jasperserver-context-xml
    readOnly: true
  {{- end }}
