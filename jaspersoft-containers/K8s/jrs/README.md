...
| scalable-query-engine.drivers.image.tag                          | Scalable Query Engine Drivers image tag                                                                                                                                 | 10.0.0                                                                                                                                                                       |
...
## Cache Configuration
  JasperReports® Server uses Infinispan as its default caching mechanism to enhance performance and reduce database load.
Before building the Docker images you can review and adjust the cache configuration in the default_master.properties file.
appCacheType=infinispan-embedded
For single-node installations, update the configuration as shown below:
appCacheType=infinispan-local
This ensures the appropriate caching mode is used based on your deployment setup..
